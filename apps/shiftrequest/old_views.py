        ...
        with transaction.atomic():
            try:
                ...
                emp_req = EmployeeRequests.objects.create(
                    ...
                    year=__year,
                    month=__month,
                    request_data={'availability': {str(day): 'green' for day in range(1, calendar.monthrange(__year, __month)[1] + 1)}}
                )
            except IntegrityError:
                pass
    else:
        emp = Employee.objects.filter(user=request.user, year=__year, month=__month).first()
